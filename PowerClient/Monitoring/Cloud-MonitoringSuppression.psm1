...
function Add-CloudMonitoringSuppression {
    ...
    Write-Debug "Body: `n$suppressionBody"
    try {
        Invoke-RestMethod -Uri $suppressionUri -Body $suppressionBody -ContentType application/json -Headers (Get-HeaderDictionary) -Method Post
    } catch {
...
<#
    ...
    At tihs point, there is no validation for this parameter and no conversion functaionlity. Future releases
    may include more helpful information/functionality.
...
function Convert-CloudMonitoringSuppression {
    ...
    if($startTime) { $body | Add-Member -MemberType NoteProperty -Name start_time -Value ([int64]$startTime) }
    if($endTime) { $body | Add-Member -MemberType NoteProperty -Name end_time -Value ([int64]$endTime) }
...
function Get-CloudMonitoringSuppressions {
    ...
    return (Get-CloudMonitoringSuppression)
<#
    .SYNOPSIS
...
function Remove-CloudMonitoringSuppression {
    param (
...
function Update-CloudMonitoringSuppression {
    ...
    try {
        Invoke-RestMethod -Uri $suppressionUri -Body $suppressionBody -ContentType application/json -Headers (Get-HeaderDictionary) -Method Put
    } catch {
        Write-Host "Generic Error Here"
    }
<#
